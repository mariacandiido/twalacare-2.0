...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (email: string, password: string, userType: UserType): Promise<boolean> => {
    try {
      setIsLoading(true);
      setError(null);
      const result = await authService.login({ email, password, tipo: userType });
  ...
  const sendVerificationEmail = async (email: string) => true;
  const verifyEmail = async (token: string) => true;
