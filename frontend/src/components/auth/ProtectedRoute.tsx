...
export function ProtectedRoute({ children, allowedTypes }: ProtectedRouteProps) {
  ...
  // Devolve o caminho padrão de cada tipo de utilizador quando o acesso é negado.
  // Cada tipo tem a sua própria "página inicial" dentro da plataforma.
  const getDashboardPath = (tipo: UserType) => {
    switch (tipo) {
      case "farmacia":
        return "/farmacia/dashboard";          // painel da farmácia
      case "entregador":
        return "/entregador/dashboard";        // painel do entregador
      case "admin":
        return "/admin/dashboard";             // painel do administrador
      case "cliente":
        return "/cliente/historico-compras";   // página inicial do cliente
      default:
        return "/";
    }
  };
