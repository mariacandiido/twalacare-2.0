...
function Step2({ step1Data, onBack }: { step1Data: Step1Data; onBack: () => void }) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      formData.append("numeroBi", numeroBi);
      formData.append("dataNasc", dataNasc);
      formData.append("endereco", endereco);
      formData.append("provincia", provincia);
      formData.append("municipio", municipio);
      formData.append("bairro", bairro);
      formData.append("marcaMoto", marcaMoto);
      formData.append("modeloMoto", modeloMoto);
      if (corMoto) formData.append("corMoto", corMoto);
      if (anoMoto) formData.append("anoMoto", anoMoto);
      formData.append("matricula", matricula);
      if (banco) formData.append("banco", banco);
      if (iban) formData.append("iban", iban);

      if (docBi?.file) formData.append("docBi", docBi.file);
      if (docCarta?.file) formData.append("docCarta", docCarta.file);
      if (docLivrete?.file) formData.append("docLivrete", docLivrete.file);
      if (docFoto?.file) formData.append("docFoto", docFoto.file);

      const result = await authService.completeEntregadorRegistration(formData);
      if (!result.success) {
