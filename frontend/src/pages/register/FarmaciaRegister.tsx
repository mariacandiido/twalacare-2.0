      ...
      <div className="w-full max-w-lg rounded-2xl p-8 my-6"
        style={{ backgroundColor: "#fff", boxShadow: "0 8px 40px rgba(0,0,0,0.08)", border: "1px solid rgba(44,85,48,0.08)" }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-5">
            ...
            />
            {confirmPassword && (
              <p style={{ fontFamily: "'Roboto',sans-serif", fontSize: 12, color: password.trim() === confirmPassword.trim() ? GREEN : RED }}>
...
function Step2({ step1Data, onBack }: { step1Data: Step1Data; onBack: () => void }) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      formData.append("licencaFunc", licencaFunc);
      formData.append("registroComercial", registroComercial);
      formData.append("licencaSanitaria", licencaSanitaria);
      formData.append("farmNome", farmNome);
      formData.append("farmCedula", farmCedula);
      formData.append("farmTel", farmTel);
      formData.append("provincia", provincia);
      formData.append("municipio", municipio);
      formData.append("bairro", bairro);
      formData.append("rua", rua);
      if (numEdificio) formData.append("numEdificio", numEdificio);
      if (referencia) formData.append("referencia", referencia);

      if (docLicencaFunc?.file) formData.append("docLicencaFunc", docLicencaFunc.file);
      if (docAlvara?.file) formData.append("docAlvara", docAlvara.file);
      if (docRegistroComercial?.file) formData.append("docRegistroComercial", docRegistroComercial.file);
      if (docLicencaSanitaria?.file) formData.append("docLicencaSanitaria", docLicencaSanitaria.file);
      if (docNif?.file) formData.append("docNif", docNif.file);
      if (docCedula?.file) formData.append("docCedula", docCedula.file);

      const result = await authService.completeFarmaciaRegistration(formData);
      if (!result.success) {
        setToast({ type: "error", msg: result.error || "Não foi possível enviar os documentos" });
        return;
      }
      ...
      console.error(err);
      setToast({ type: "error", msg: "Erro ao enviar o cadastro. Tente novamente." });
    } finally {
      setIsLoading(false);
    }
  };
