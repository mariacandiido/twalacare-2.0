...
import type { ItemPedidoFarmacia } from "../../types/farmacia.types";
import type { OrderStatus } from "../../types";
import { useAuth } from "../../hooks/useAuth";
import { useAdminStore } from "../../store/adminStore";

// Vista unificada para a farmácia (dados vêm do store partilhado com o admin)
type PedidoFarmaciaView = {
  id: string;
  nomeCliente: string;
...
const STATUS_CONFIG: Record<string, { label: string; bg: string; color: string; icon: React.ElementType }> = {
  ...
  "pronto-entrega": { label: "Pronto para entrega", bg: "rgba(44,85,48,0.1)", color: "#2c5530", icon: CheckCircle },
  ...
  cancelado: { label: "Recusado/Cancelado", bg: "rgba(212,90,90,0.12)", color: "#d45a5a", icon: XCircle },
  recusado: { label: "Recusado", bg: "rgba(212,90,90,0.12)", color: "#d45a5a", icon: XCircle },
};
...
function parseItensString(itensStr: string, total: number): ItemPedidoFarmacia[] {
  if (!itensStr.trim()) return [{ produtoId: "", nomeProduto: itensStr || "—", quantidade: 1, precoUnitario: total }];
  return itensStr.split(",").map((parte, i) => {
    const match = parte.trim().match(/^(.+?)\s*×\s*(\d+)$/);
    const nome = match ? match[1].trim() : parte.trim();
    const quantidade = match ? parseInt(match[2], 10) : 1;
    return { produtoId: String(i), nomeProduto: nome, quantidade, precoUnitario: Math.round(total / (match ? parseInt(match[2], 10) : 1)) };
  });
}

export function FarmaciaPedidos() {
  const { user } = useAuth();
  const { pedidos: pedidosAdmin, atualizarStatusPedido } = useAdminStore();
  const [search, setSearch] = useState("");
  const [filtroStatus, setFiltroStatus] = useState<string>("todos");
  const [detalheId, setDetalheId] = useState<string | null>(null);

  const pedidos: PedidoFarmaciaView[] = useMemo(() => {
    const nomeFarmacia = user?.nome ?? "";
    return pedidosAdmin
      .filter((p) => p.farmacia === nomeFarmacia)
      .map((p) => ({
        id: p.id,
        nomeCliente: p.clienteNome,
        telefoneCliente: "—",
        enderecoEntrega: p.enderecoEntrega,
        itens: parseItensString(p.itens, p.total),
        total: p.total,
        status: p.status,
        dataPedido: p.dataPedido,
        metodoPagamento: p.metodoPagamento,
      }));
  }, [pedidosAdmin, user?.nome]);
  ...
  function avancarStatus(id: string) {
    ...
    if (!p) return;
    const proximo = NEXT_STATUS[p.status];
    if (proximo) atualizarStatusPedido(id, proximo);
  }
  ...
  const totais = {
    ...
    "pronto-entrega": pedidos.filter((p) => p.status === "pronto" || p.status === "pronto-entrega").length,
  };
        ...
        <div className="space-y-3">
          {filtered.length === 0 ? (
            ...
            filtered.map((pedido) => {
              ...
              const podeAvancar = nextStatus[pedido.status];
              return (
                <div
                  key={pedido.id}
                    ...
                    <div className="flex items-start gap-4">
                      <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg, #2c5530, #4a7856)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <span style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, color: "#ffffff", fontSize: 16 }}>
                          {pedido.nomeCliente.charAt(0)}
                        </span>
                      </div>
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 600, color: "#2c3e2c", margin: 0 }}>{pedido.nomeCliente}</p>
                          <span style={{ fontFamily: "monospace", fontSize: 12, color: "#888" }}>#{pedido.id}</span>
                        </div>
                        <p style={{ fontFamily: "'Roboto', sans-serif", fontSize: 13, color: "#4a7856", marginTop: 2 }}>
                          {pedido.itens.map((i) => `${i.nomeProduto} ×${i.quantidade}`).join(" · ")}
                        </p>
                        <p style={{ fontFamily: "'Roboto', sans-serif", fontSize: 12, color: "#888", marginTop: 2 }}>
                          {formatarData(pedido.dataPedido)} · {pedido.metodoPagamento}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {podeAvancar && (
                          ...
                          >
                            ...
                            {pedido.status === "pendente" || pedido.status === "confirmado" ? "Iniciar preparo" : "Marcar Pronto"}
                          </button>
                        )}
          ...
          >
            ...
            <div style={{ padding: 24 }} className="space-y-5">
              <div>
                ...
                <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 600, color: "#2c3e2c", margin: 0 }}>{pedidoDetalhe.nomeCliente}</p>
                <p style={{ fontFamily: "'Roboto', sans-serif", fontSize: 13, color: "#4a7856", margin: "2px 0" }}>{pedidoDetalhe.telefoneCliente}</p>
                <p style={{ fontFamily: "'Roboto', sans-serif", fontSize: 13, color: "#4a7856" }}>{pedidoDetalhe.enderecoEntrega}</p>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {pedidoDetalhe.itens.map((item, i) => (
                    <div key={i} className="flex items-center justify-between">
                      <span style={{ fontFamily: "'Roboto', sans-serif", fontSize: 14, color: "#2c3e2c" }}>{item.nomeProduto} ×{item.quantidade}</span>
                      <span style={{ fontFamily: "'Roboto', sans-serif", fontWeight: 500, color: "#2c3e2c" }}>{(item.quantidade * item.precoUnitario).toLocaleString()} Kz</span>
                    </div>
                  ))}
                  <div className="flex items-center justify-between" style={{ paddingTop: 8, borderTop: "1px solid #e5e5e5" }}>
                    ...
                    <span style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, color: "#2c5530" }}>{pedidoDetalhe.total.toLocaleString()} Kz</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <p style={{ fontFamily: "'Roboto', sans-serif", fontSize: 14, color: "#2c3e2c" }}>{pedidoDetalhe.metodoPagamento}</p>
              </div>
            ...
            <div className="flex gap-3" style={{ padding: "16px 24px", borderTop: "1px solid #f0f0f0" }}>
              {pedidoDetalhe.status === "pendente" && (
                <>
                  <button onClick={() => recusarPedido(pedidoDetalhe.id)} className="twala-btn-outline flex-1" style={{ borderColor: "#d45a5a", color: "#d45a5a" }}>
                    Recusar
                  </button>
                  <button onClick={() => { avancarStatus(pedidoDetalhe.id); setDetalheId(null); }} className="twala-btn-primary flex-1">
                    Confirmar Pedido
                  </button>
                </>
              )}
              ...
              {(pedidoDetalhe.status === "pronto" || pedidoDetalhe.status === "pronto-entrega" || pedidoDetalhe.status === "recusado" || pedidoDetalhe.status === "cancelado" || pedidoDetalhe.status === "entregue" || pedidoDetalhe.status === "em-transito") && (
