...
- **Repositories**: acesso a dados (Prisma); um repositório por entidade principal.
- **Models**: definido no Prisma schema (`prisma/schema.prisma`) com mapeamento de relações.
...
├── prisma/                # esquema Prisma e arquivos de migração
├── jest.config.js
├── Dockerfile
├── docker-compose.yml
└── README.md
...
- **MySQL**: Prisma usa `DATABASE_URL` em `.env` (ex.: `mysql://user:password@host:port/dbname`).
- **Servidor**: `src/config/server.js` (PORT, NODE_ENV).
- **JWT**: `src/config/jwt.js` (JWT*SECRET, JWT_EXPIRES_IN, JWT_REFRESH*\*).
...
- **express-rate-limit** (configurável por RATE*LIMIT*\*).
