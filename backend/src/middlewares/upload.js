...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    if (file.fieldname === 'medicamento_imagem') {
      ...
      cb(null, recDir);
    } else if (file.fieldname.startsWith('doc')) {
...
const fileFilter = (req, file, cb) => {
  ...
  if (file.fieldname === 'receita_arquivo' || file.fieldname.startsWith('doc')) {
    if (allowedDocTypes.test(extname)) return cb(null, true);
    return cb(new Error('Formato de arquivo não suportado. O documento deve ser PDF ou Imagem.'));
  }
