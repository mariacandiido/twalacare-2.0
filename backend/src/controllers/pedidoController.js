...
const updateStatus = asyncHandler(async (req, res) => {
  ...
  if (req.user.tipo === "cliente" && !allowedClient.includes(status)) {
    return res
      .status(403)
      .json({
        status: "error",
  ...
  if (req.user.tipo === "farmacia") {
    ...
    if (!allowedFarmacia.includes(status)) {
      return res
        .status(403)
        .json({
          status: "error",
  ...
  if (req.user.tipo === "admin" && !allowedAdmin.includes(status)) {
    return res
      .status(403)
      .json({ status: "error", message: "Status inválido para admin" });
  }

  const atualizado = await pedidoRepository.updateStatus(req.params.id, status);
