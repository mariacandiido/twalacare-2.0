...
exports.registerFarmacia = catchAsync(async (req, res) => {
  const result = await authService.register(req.body, 'FARMACIA', req.files);
...
exports.registerEntregador = catchAsync(async (req, res) => {
  const result = await authService.register(req.body, 'ENTREGADOR', req.files);
  ApiResponse.success(res, 'Entregador registado com sucesso', result, 201);
});

exports.completeFarmaciaRegistration = catchAsync(async (req, res) => {
  const result = await authService.completeFarmaciaRegistration(req.body, req.files);
  ApiResponse.success(res, 'Cadastro da farmácia atualizado com sucesso', result);
});

exports.completeEntregadorRegistration = catchAsync(async (req, res) => {
  const result = await authService.completeEntregadorRegistration(req.body, req.files);
  ApiResponse.success(res, 'Cadastro do entregador atualizado com sucesso', result);
});
